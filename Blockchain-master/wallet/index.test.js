...
describe('Wallet' , () =>{
    ...
    describe( 'calculating a balance' , () => {
     ...
     it('calculates the balance for the blockchain transaction matchiung th recipient' ,() => {
         expect(wallet.calculateBalance(bc)).toEqual(INITIAL_BALANCE + (addBalance * repeatAdd));

     });

     it('calculates the balance for the blockchain transaction matchiung th sender' ,() => {
    ...
    describe('The recipient conducts a transaction' ,() => {
        let subBalance , rBalance;

        beforeEach(()=>{
            tp.clear();
            subBalance = 60;
            rBalance = wallet.calculateBalance(bc);
            wallet.createTransaction(senderWallet.publicKey ,subBalance,bc,tp);
            bc.addBlock(tp.transactions);
        });
        ...
        /*    it('calculate the recipient balance only using transactions since its most recent one',() =>{
                expect(wallet.calculateBalance(bc)).toEqual(rBalance -subBalance+addBalance);
            });*/
        });
    });
